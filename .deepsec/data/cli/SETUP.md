...
This is a deepsec scanning workspace. Project `cli` was just registered
(target: `..`). Setup is incomplete — `data/cli/INFO.md`
...
2. **Fill in `data/cli/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id cli
pnpm deepsec process --project-id cli
```
